...
# ----------------------------------------------------------------------
# Content filter (unchanged – kept for safety)
# ----------------------------------------------------------------------
class ContentFilter:
    def __init__(self):
        self.bad = [
...
class UserSession:
    ...
    def load_profile(self):
        ...
        # 3. email
        if "@" in self.user_id:
            p = get_user_profile_by_email(self.user_id)
            if p:
                self.profile = p
                return
...
async def chat(req: ChatRequest):
    ...
    try:
        ...
        answer = f"Sorry {sess.first_name()}, I hit a snag. Try again in a moment."
...
def _base(req: ChatRequest, sess: UserSession | None = None):
    ...
    return {
        "user_id": req.user_id,
        "username": sess.first_name(),
        "full_name": sess.first_name(),
